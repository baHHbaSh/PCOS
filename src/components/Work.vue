<template>
	<div class="owerlay">
		<h1 class="owerlayData">$ - {{CollectedMoney}}</h1>
	...
	<div v-for="(war, k) in War" :key="k">
		<div class="challenge">
			...
			<p>x=</p>
			<div class="row">
				<button @click="Answer(answer, war[2])" v-for="(answer, p) in shuffleArray(war[1])" :key="p">{{answer}}</button>
			</div>
		</div>
	</div>
...
<script>
	...
	export default{
		...
		methods:{
			...
			Answer(Ans, x){
				if(Ans == x) this.CollectedMoney+=this.CPU**2
				else this.Lives--
				this.War.pop()
			...
			GetChallenge(){
				this.War.push(this.QuizGen(this.Randint(1,this.CPU)))
			},
			QuizGen(Hard){
				if(Hard == 1){
